...
#include "fa/translator/fa_translator.h"
...
class FaTranslateProvider final
...
public:
	...
	: _session(session)
	, _provider(provider) {
		if (!Fa::Translator::TranslateManager::currentInstance()) {
			Fa::Translator::TranslateManager::init();
		}
	}
	...
	void requestBatch(
		...
		const auto manager = Fa::Translator::TranslateManager::currentInstance();
...
std::unique_ptr<TranslateProvider> CreateFaTranslateProvider(
		not_null<Main::Session*> session,
		TranslationProvider provider) {
	return std::make_unique<FaTranslateProvider>(session, provider);
}
