...
void HistoryWidget::send(Api::SendOptions options) {
	...
	message.textWithTags = applyAutoParseMarkdownHyperlink(_field->getTextWithAppliedMarkdown());
...
void HistoryWidget::sendScheduled(Api::SendOptions initialOptions) {
	...
	if (showSendMessageError(
			applyAutoParseMarkdownHyperlink(_field->getTextWithAppliedMarkdown()),
			ignoreSlowmodeCountdown)) {
		return;
	}
	controller()->show(
